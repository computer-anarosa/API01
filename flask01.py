...
def delete(id):
    ...
    try:
       ...
       # Retorna com mensagem de sucesso e status HTTP "200 Ok not returned".
        return {"success": "Registro apagado com sucesso", "id": id}, 200
...
@app.route("/owners/items/<owner_id>", methods=["GET"])  
def listar_itens_usuario(owner_id):
    try:
     conn = sqlite3.connect(database)
     conn.row_factory = sqlite3.Row
     cursor = conn.cursor()
...
@app.route("/owners/items/user/<item_id>", methods=["GET"])
def listar_item_com_proprietario(item_id):
    try:
        conn = sqlite3.connect(database)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        ...
        sql = """
            SELECT item.*, owner_name, owner_email
            FROM item
            JOIN owner ON item_id = owner_id
            WHERE item_id = ?
        """
        ...
        if item:
            ...
            # Remove prefixos dos campos do item, se necessário

            # Retorna o item com dados do proprietário
